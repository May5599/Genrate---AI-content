...
// Twitter: Generate a thread with concise tweets under 280 characters
if (contentType === "twitter") {
  promptText += " Create a Twitter thread with 5 tweets, each under 280 characters. Ensure each tweet is engaging and connected. Do not include any extra explanations or numbering.";
} 

// Instagram: Ensure each post has engaging captions without unnecessary filler
else if (contentType === "instagram") {
  promptText += " Generate 5 separate Instagram posts with engaging captions. Keep them concise, creative, and relevant to the topic. Avoid unnecessary filler text.";
} 

// LinkedIn: Ensure long-form, professional, well-structured paragraphs
else if (contentType === "linkedin") {
  promptText += " Generate 5 LinkedIn posts, each in a well-structured paragraph format. Ensure the content is professional, insightful, and engaging. Avoid unnecessary line breaks and make the writing flow naturally.";
}

let imagePart: Part | null = null;
if ((contentType === "instagram" || contentType === "linkedin") && image) {
...
// Filter out extra text and keep only valid posts
let content: string[] = [];
if (contentType === "twitter" || contentType === "instagram" || contentType === "linkedin") {
  content = generatedText
    .split("\n\n")
    .filter((post) => post.trim() !== "" && !post.toLowerCase().includes("explanation") && !post.toLowerCase().includes("note")); // Exclude explanation or note text
} else {
  content = [generatedText];
}

// Render LinkedIn content in a Markdown-styled box
if (contentType === "linkedin" && content.length > 0) {
  return (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gray-800 p-6 rounded-2xl flex items-center justify-between">
              <div className="flex items-center">
                <Send className="h-8 w-8 text-yellow-400 mr-3" />
                <div>
            ...
            {(selectedHistoryItem || generatedContent.length > 0) && (
              ...
              {contentType === "twitter" || contentType === "instagram" || contentType === "linkedin" ? (
                <div className="space-y-4">
                  {(selectedHistoryItem
